...
import sys
from UBC_test import test_on_train
...
def train(model, train_loader, device, optimizer, epoch):
    ...
    for batch_idx, (input_mb_loc, input_mb_glo, lbl) in enumerate(train_loader):
        print(batch_idx + 1, end=", ", flush=True)
        input_mb_loc = input_mb_loc.to(device) 
        input_mb_glo = input_mb_glo.to(device) 
        lbl = lbl.to(device) 
    ...
    nb_mb_it = (len(train_loader.dataset) // input_mb_loc.shape[0])
    train_loss /= nb_mb_it
    loss_dict = {k:v / nb_mb_it for k, v in loss_dict.items()}
    print(loss_dict)
    return train_loss, input_mb_loc, input_mb_glo, recon_mb, loss_dict, lbl


def eval(model, test_loader, device):
    model.eval()
    input_mb_loc, input_mb_glo, gt_mb = next(iter(test_loader))
    gt_mb = gt_mb.to(device)
    input_mb_loc = input_mb_loc.to(device)
    input_mb_glo = input_mb_glo.to(device)
    recon_mb, opt_out = model(input_mb_loc, input_mb_glo)
    rec_loss = (
                torch.square(
                    input_mb_loc - recon_mb,
                )
            ).detach()
    recon_mb = model.mean_from_lambda(recon_mb)
    return input_mb_glo, input_mb_loc, recon_mb, gt_mb, opt_out
...
def main(args):
    ...
    try:
        ...
        for epoch in range(args.num_epochs):
            print("Epoch", epoch + 1)
            loss, input_mb_loc, input_mb_glo, recon_mb, loss_dict, lbl = train(
                model=model,
            ...
            if (epoch + 1) % 50 == 0 or epoch in [0, 4, 9, 14, 19, 24, 29, 49]:
                ...
                model.eval()
                input_glo_test_mb, input_loc_test_mb, recon_test_mb, _, opt_out = eval(model=model,
